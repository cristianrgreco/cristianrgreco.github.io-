...
:page-tags: [kubernetes, ansible, flux]

This post will show you how to install and configure Kubernetes using https://microk8s.io/[MicroK8s] on your home server using https://www.ansible.com/[Ansible]. We'll be using https://fluxcd.io/[Flux] for managing app deployments and cluster configuration. This is a great way to get hands-on experience with Kubernetes and CI/CD best practices like GitOps.

[NOTE]
====
We will cover deployments via Flux such as Prometheus/Grafana/Loki for observability, MetalLB for load balancing, and installing apps like Minecraft server and PiHole, in future posts.
====
...
. SSH public key authentication set up between the server and your host.
...
Flux will have created a GitHub repository in your account. Clone it down and have a look around. Soon we will use it for deploying apps and cluster configuration. On the host you can run `flux events` to see what Flux is up to.
...
spec:
  ...
  template:
    metadata:
      ...
      namespace: observability
----
. https://microk8s.io/docs/addon-hostpath-storage#:~:text=By%20default%2C%20the%20hostpath%20provisioner,%2Fcommon%2Fdefault%2Dstorage%20.[MicroK8s host storage path addon]. This will let Kubernetes provision volumes using host storage. This is what mine looks like after having installed a few apps:
+
[source,bash]
----
...
Stay tuned for the next post where we'll deploy Prometheus, Grafana and Loki, with custom alerts that will email us when our host CPU/memory/disk utilisation, and even temperature, is too high. We'll end up with something like:
