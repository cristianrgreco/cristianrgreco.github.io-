...
Success! The sealed secrets pod was able to decrypt the password.

=== Summary

We're able to get our secret with the password `super-secret-password` into the cluster from our sealed secret, which contains the encrypted password. The sealed secret can be checked into source control and even made public.
